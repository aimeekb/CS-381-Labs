...
sub build_bigrams {
	...
	for @tracks -> $title {
		...
		my @term = split(" ", $_).Array;
		if (@term.elems > 1) {
			my $var = 0;
			#While it is not the last word
			while $var != @term.elems - 1 {
				#Check to make sure that the word exists, if it does add one
				if (%counts{@term[$var]}{@term[$var +1]}:exists) {
					%counts{@term[$var]}{ @term[$var + 1]}++;
				}
				#If it doesn't create it and add it to the array.
				else {
					%counts{@term[$var]}{ @term[$var + 1]}++;
				}
				$var++;
	...
	#Return the array that contains the hash of hashes.
	return %counts;
...
sub mcw {
	...
	#If the word exists in the array enter this function
	if (%counts{$word}) {
		my $maxvalue = 0;
		my $value = 0;
		my $maxkey = "";
		my $word_not;
		for %counts{$word}.sort -> (:key($k), :value($v)) {
			#Check to make sure its higher than the last value.
			#Double check to make sure that the key is not in the word history.
			if ($v > $maxvalue && not %word_history{$k}:exists) {
				$value++;
				$maxvalue = $v;
				$maxkey = $k;
			}
		}
		#If the word is not empty then set the word.
		if ($maxkey[0] ne "") {
			$best_word = $maxkey[0];
		}
	}

	#Return this if the %counts{$word} is not found.
	else {
		return "";
	}
...
sub sequence {
	...
	#Set the first word equal to the first parameter
	my $first_word = @_[0];
	#add it to the song title
	my $song_title =  $first_word;
	#Add the first word to the word history array.
	%word_history{$first_word}++;
	my $word = mcw ($first_word);
	#Get the next word
	my $length = 1;
	#If nothing an empty string is returned or a newline is returned return $song_title
	if ($word eq "" || $word eq "\n") {
		return $song_title;
	}
	my $new_word;
	...
	while $length < $SEQUENCE_LENGTH {
		#Add $word to the song_title
		$song_title = $song_title ~ " " ~ $word;
		#Add $word to the %word_history
		%word_history{$word}++;
		$new_word = $word;
		#Get a new word.
		$word = mcw ($new_word);
		#Return if the $word is empty or its a newline.
		if ($word eq "" || $word eq "\n") {
			return $song_title;
		}
		#Increase the length
